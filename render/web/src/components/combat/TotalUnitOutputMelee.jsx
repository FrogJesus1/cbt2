 ...
 *
 * Melee aggregate output card.
 * Layout mirrors TotalUnitOutputRanged — see that file for shape description.
 */
...
export function TotalUnitOutputMelee({ data, maxDmg = 10 }) {
  ...
  return (
    <Card style={CARD_STYLE}>
      <CardContent style={CARD_PAD}>
        <SectionTitle>Melee Output</SectionTitle>
        {items.length > 0
          ? (
            <>
          ...
          : <div style={{ color: C.label, fontSize: "12px", fontStyle: "italic" }}>No melee data.</div>
        }
      </CardContent>
    </Card>
  );
