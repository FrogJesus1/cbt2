...
**Status:** Phases 0–7 COMPLETE & validated. **All redesign phases landed** — a holistic visual test/fix pass is the remaining step. Decisions A/B/C/D/E/F/G/H all resolved (A: exact mockup look, kept themeable · B: solid bar variant · C: mobile out of scope · D: D2 full manual override · E: E2 display preference · F: F1 real aliases · G: G1 rules endpoints · H: 11TH ED cosmetic).
...
| **E** ✅ **RESOLVED** | **Settings → Math Mode** — is the MC/Deterministic + iterations + confidence panel a **real engine control** or display-only (as in the mockup)? | (E1) Wire to real engine params (`simulation_config.trials`, mode). (E2) Display-only chrome for now. | **DECIDED: E2 (display preference).** The recommendation was "E1 if low-effort, else E2" — and it is **not** low-effort/safe: `get_mc_config`/`set_mc_trials` are **global module state** (no per-session config) with **no API setter**, and they feed the **load-bearing combat math** shared across all sessions on the live deployment. So the panel is a display/preference surface: it shows the engine's **real** current mode + trials (read from `/api/engines/{id}` status), persists the user's tuning to `localStorage ct_math_prefs`, and the **Edition** control is the one real, persisted (cosmetic per H) wiring via `useEdition`. | Settings only. |
| **F** ✅ **RESOLVED** | **Aliases tab** — no backend exists. | (F1) Build real (localStorage `ct_aliases` + Terminal expansion + profile sync). (F2) Static placeholder. | **DECIDED: F1 (real).** New `lib/aliases.js` (read/write/expand) + `localStorage ct_aliases`; expansion runs at the single App command chokepoint (`handleGlobalCommand`) so typed/injected/animated commands all benefit; `alias <short> <expansion>` typed in the bar creates one; the Aliases tab is the CRUD surface. **Profile-sync deferred** (noted) — the local store is the source of truth for now. | Settings only. |
| **G** ✅ **RESOLVED** | **Rules data delivery** — no endpoint serves the full rules/keyword dataset to the client. | (G1) **New `GET /api/rules` + `/api/keywords`** endpoints. (G2) Bundle a generated JSON into the frontend. | **DECIDED: G1.** Both endpoints added (server.py, loaded-once + cached, graceful empty fallback). Browser merges the two real files client-side. Single source of truth — never drifts from the engine data. | Rules browser. |
...
### Phase 6 — Settings + Rules ✅ COMPLETE (2026-06-19)

**Shipped:** *Decisions* — **E2** (Math Mode = display preference, not a global engine mutator), **F1** (real `ct_aliases`), **G1** (`/api/rules` + `/api/keywords`); see the decisions table for the reasoning. *Server* — `server.py` gained `GET /api/rules` + `GET /api/keywords` (108 rules / 61 keywords served from the engine's own `rules.json` + `keyword_dictionary.json`, loaded-once + cached, graceful empty fallback) — the single source of truth for the Rules browser. *New `lib/aliases.js`* — read/write/expand for user aliases (`ct_aliases`), dedupe-by-alias, first-token + whole-string expansion, change events. *New `components/SettingsPage.jsx`* — a 4-sub-tab page (Chakra sub-tab bar, green active underline) replacing the old hidden `settings` terminal: **Diagnostics** embeds the existing `DiagnosticsPage` (no double-mount — the standalone `diag` context is **removed** and folded in here); **Command List** is **LIVE** from `/api/engines/{id}/schema`, grouped by the engine's own command `group`s with per-group accent dots + `usage`/`description` rows (stubs hidden); **Math Mode** (E2) = Rules-Edition segmented (real, via `useEdition`) + Engine-Mode/iterations/confidence/show-working/variance controls that **seed from the engine's live status** (`simulation_mode` + `simulation_config.trials`) and persist to `ct_math_prefs`, with a Current-Config preview + sample ledger + an honest "engine is live in …" line; **Aliases** (F1) = `ct_aliases` CRUD (add/remove, Enter-to-add, empty state). *Rewrote `RulesContext.jsx`* from a thin Terminal wrapper into the **rules browser** (Rules.dc.html): fetches both endpoints, **merges + dedupes** rules↔keywords into one categorised list (a keyword that defines/echoes a rule inherits that rule's effect text while keeping its class → category + `math_relevant` → ✓/~ SIM badge; 56/61 keywords merge, 123 unique entries), then renders title + search, a category rail with live counts (Weapon/Keyword/Phase/Unit/Core/Stratagem/Detachment/Mission), a **dynamic Source filter** (shown only when the current view spans ≥2 sources — a faithful-to-real-data substitute for the mockup's faction filter, which the real dataset can't populate), a "Sim-relevant only" toggle, and expandable cards (interactions / parameter / timing / conditions / applies-to). *App.jsx wiring* — new `settingsTab` state; the SETTINGS dropdown items now **set context+tab directly** (no command injection) with per-tab active states; `handleGlobalCommand` gained F1 alias expansion at the top + `alias <short> <expansion>` creation + single-word routing for `diag`/`diagnostics`/`commands`/`aliases`/`settings` → Settings tabs + `rule <term>`/`rules <term>` → the Rules browser (seeds search, auto-opens the exact match); `diag` removed from `NAV_CMD_MAP` and redirected in `handleNavigate`; `NON_TERMINAL` now `{rosters, crusade, settings, rules}`; removed the dead hidden-settings Terminal panel + `SETTINGS_BOOT_LINES` + `openInMain` + the standalone `diag` panel/`DiagnosticsPage` App import.

**Reuse honored:** no router / no `?param`; the engine `rule_block` path + `TerminalRuleBlock.jsx` are left intact (typed lookups still render that block in any terminal context); the existing `math`/`mathmode` **ledger-replay** (`MathModeBlock`, Terminal-level) is untouched — Math Mode *settings* is a separate surface reached via the dropdown. All colours flow through `C.*`/CSS vars (only fixed non-semantic values are the decorative category tints — purple `#a78bdb`, slate `#8fb0c4`, orange `#e08a5a`, steel `#6fb1c9` — matching the established palette).

**Verified:** esbuild parse of all changed/new JS (App, SettingsPage, RulesContext, lib/aliases) + full import-graph bundle from `App.jsx` **and** the SettingsPage + RulesContext subtrees resolve clean, **no errors**. **159 core tests** (loader + combat math) + **13 API-integration tests** pass after the `server.py` change; the two new endpoints were smoke-tested through the real FastAPI app (`/api/rules` → 108, `/api/keywords` → 61, `math_relevant` flags intact). Live `vite build`/browser screenshot still pending on the dev machine (sandbox mount limit); holistic visual pass scheduled after all phases land.

**Deferred / noted:** (1) alias **profile-sync** (F1 mentioned it) — left for later; `ct_aliases` is local-only for now. (2) The Rules **faction filter** from the mockup is replaced by a **Source filter** — the real rules dataset is core-rules-centric and not faction-scoped, so a faction dropdown would be empty; the category rail + sim toggle + search are the functional filters. (3) Math Mode is **display-only** (E2) by design — wiring it to the engine would mean mutating global MC state for every session, out of scope. (4) Typing `theme <name>` while on a non-terminal tab (settings/rules) routes to main to apply it; the always-present nav theme **dropdown** changes theme in-place, so this is a non-issue in practice. Logged to `BACKLOG.md`.

### Phase 6 — Settings + Rules *(original spec below)*
...
### Phase 7 — Login + boot + Error/Loading motion ✅ COMPLETE (2026-06-19)

**Shipped:** *Foundation* — `index.css` gained the Phase-7 motion vocabulary, all `prefers-reduced-motion`-gated: CRT `.ct-crt-scanlines`/`.ct-crt-vignette` overlays, idle `.ct-flicker` + one-shot `.ct-power-on` + `.ct-boot-scan` (boot), `.ct-ctx-flicker`/`.ct-ctx-sweep` (context switch), `.ct-sim-track` (indeterminate "simulating" bar), `.ct-reveal` (result fade+rise), `.ct-pulse` (status dots). New `hooks/useCountUp.js` (`useCountUp` + `<CountUp>`, easeOut, reduced-motion → snaps to final, null → dash).
*Login* — `ProfileGate.jsx` **fully rewritten** to `Login.dc.html`: a CRT cogitator-boot console (⚡ COMBAT TERMINAL banner + live `useEdition` 11TH ED chip + streamed boot log `cogitator core OK / dossiers / monte carlo ONLINE` → `▸ READY`), then the auth body (pick / PIN / create / skip) recreated as `PickView`/`PinView`/`CreateView` — status dot gold=PIN·green=open, `🔒 PIN` badge, custom PIN checkbox, prompt footer with `› awaiting authentication` + blinking caret, scanline+vignette+flicker overlays. Boot motion plays **once per session** (`sessionStorage ct_booted`) and is skipped under reduced motion. Drops the old hardcoded `#39ff14`/JetBrains styling for `--ct-*` tokens (sets `data-theme` from `ct_active_theme` so the login re-hues with the active theme; the darker-than-app boot-console shades are fixed phosphor darks with no token equivalent). **All API logic preserved** (`fetchProfiles`/`loginProfile`/`createProfile`/`hydrateFromProfile`/auto-login/last-profile).
*Errors* — `App.jsx`: the old thin red `apiError` banner + "Server offline" main text are replaced by (a) a full-screen **`⚠ COGITATOR UNREACHABLE`** takeover overlaying the (still-mounted) panels — `python main.py` hint, `↻ reconnect` + `offline mode` buttons, and a live **5s auto-retry countdown** that re-probes `/api/engines`; and (b) an amber **"Monte Carlo offline · deterministic mode"** degraded banner when the engine is reachable but `ready===false`/`simulation_mode==="deterministic"`. The chrome status dot now reflects three health states (online green / degraded amber-pulse / no-link red-pulse) with a `DEGRADED`/`NO LINK` chip. New engine-health plumbing: a `loadEngines` callback (reused by retry), a `/status` snapshot (`ready`/`simulation_mode`/`trials`) refetched via a `healthNonce` on every (re)connect, and the countdown effect. `TerminalBlock.jsx` `TerminalError` restyled to the mockup — red left-accent block (`✕ command not recognised`), "Did you mean" bordered cards (`›` + `↵ N` run affordance, still injects `String(i+1)`), `Type help …` line + caret; report/teach affordances preserved; honest generic sub-line only on a true 404.
*Loading motion* — `CommandBar` (App): a `busy`-gated `⟳ simulating · N iterations` indeterminate strip (N from live `/status` trials; "deterministic pass" when det; gated on real command-exec, **not** the disabled/offline state). `Terminal.jsx`: `.ct-reveal` on each stream entry so results fade+rise on appear. Context-switch: a `key={activeContext}` `.ct-ctx-sweep` overlay in the panels container replays the green top-sweep on every nav change. `combat/HeroTiles.jsx`: the four hero numbers (Total Dmg / Slain / Squad Wipe % / Swing) now `<CountUp>` on reveal. The existing typewriter inject (`animateAndSubmit`) + `INJECT` flash are retained.
*Reuse honored:* no router / no `?param`; reuses `ct_active_theme`/`ct_edition`/`ct_cmd_history` + all profile APIs; introduces no new stores; every accent flows through `C.*`/`--ct-*` (only fixed values are the login boot-console darks, which have no token).

**Verified:** esbuild parse of all 6 changed files (App, ProfileGate, TerminalBlock, Terminal, HeroTiles, useCountUp) + full import-graph bundle from `App.jsx` **and** `ProfileGate.jsx` resolve clean, **no errors**; `index.css` brace-balanced + parses with no errors (new utilities present). **159 Python tests pass** (loader + combat math) — no `.py` touched, so the suite is unaffected by construction. Live `vite build`/browser screenshot still pending on the dev machine (sandbox mount limit) — folded into the now-due holistic visual pass.

**Deferred / noted:** (1) `offline mode` dismisses the connection-lost takeover into the normal (engine-less) shell — there's no true offline compute, so it's a "let me look around" escape hatch, not a deterministic-local fallback. (2) Count-up is wired into the combat HeroTiles as the representative hero-number surface; other count-up candidates (Spec ratings, threat metrics) are left for the visual pass to avoid blast radius. (3) The boot log counts (`29 factions · 1,284 units`) are flavour text, not live (the gate runs before the engine context); can be wired to `/api/factions` later if wanted. Logged to `BACKLOG.md`.

### Phase 7 — Login + boot + Error/Loading motion *(original spec below)*
...
**All resolved.** A/B/C/H (settled pre-Phase 0) · D → D2 (Phase 5) · E → E2, F → F1, G → G1 (Phase 6). No open decisions remain; Phase 7 (Login + motion + errors) is pure implementation.
